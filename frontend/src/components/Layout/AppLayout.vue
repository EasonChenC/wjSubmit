<template><el-container class="shell"><el-header class="header"><div class="brand"><span class="brand-mark">问</span><div><b>问卷自动化</b><small>Questionnaire Studio</small></div></div><div class="account"><span class="avatar">{{user.username?.slice(0,1).toUpperCase()}}</span><span>{{user.username}}</span><el-button link @click="logout">退出</el-button></div></el-header><el-container><el-aside width="220px"><div class="nav-caption">工作台</div><el-menu router :default-active="$route.path"><el-menu-item index="/tasks"><span class="nav-icon">▦</span>任务管理</el-menu-item><el-menu-item index="/settings/ai"><span class="nav-icon">AI</span>AI 配置管理</el-menu-item></el-menu></el-aside><el-main><router-view/></el-main></el-container></el-container></template>
