...
export const createTask=(data:{url:string;count:number;mode:'random'|'high_reliability';config?:SubmitConfig})=>request.post<ApiResponse<{task_id:string;status:string;submitted:number;total:number}>>('/api/questionnaire/submit',data);
