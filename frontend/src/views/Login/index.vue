<template>
  <div class="login">
    <div class="card box">
      <h1>问卷自动化系统</h1>
      <p class="muted">登录后管理问卷分析与提交任务</p>
      <el-form :model="form" @submit.prevent="submit">
        <el-form-item prop="username"><el-input v-model="form.username" placeholder="用户名" size="large" /></el-form-item>
        <el-form-item prop="password"><el-input v-model="form.password" type="password" show-password placeholder="密码" size="large" /></el-form-item>
        <el-checkbox v-model="remember">记住登录状态</el-checkbox>
        <el-button class="submit" type="primary" size="large" native-type="submit" :loading="loading">登录</el-button>
      </el-form>
    </div>
  </div>
</template>
...
const submit = async () => {
  ...
  else ElMessage.error(userStore.lastError || '用户名或密码错误')
}
</script>
