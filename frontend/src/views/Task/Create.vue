...
const form = reactive({
  ...
  attitude: 'positive' as 'positive' | 'negative', use_ai: localStorage.getItem('questionnaire_use_ai') === 'true', add_variation: false, variation_ratio: 15, debug: false,
})
...
const create = async () => {
  if (!questionnaire.value) {
    ElMessage.error('请先解析问卷')
    return
  }
  loading.value = true
  try {
    const task = await store.create({
      task_id: questionnaire.value.task_id, url: form.url, count: form.count, mode: form.mode,
      config: form.mode === 'high_reliability' ? {
        attitude: form.attitude, add_variation: form.add_variation,
        variation_ratio: form.variation_ratio / 100, debug: form.debug,
      } : { attitude: 'positive', add_variation: false, variation_ratio: 0.05, debug: form.debug },
    })
...
<style scoped>.form{max-width:650px;margin:40px auto}.el-steps{margin-bottom:36px}.el-button{margin:20px 8px 0 0} .variation-hint{margin:6px 0 0 24px;color:#8792a6;font-size:12px;line-height:1.5}.variation-option{margin-top:8px}.ratio-field{margin:14px 0 0 24px}.ratio-unit{margin-left:8px;color:#657085}.debug-switch-item{margin-top:8px}.debug-hint{margin:6px 0 0;color:#8792a6;font-size:12px;line-height:1.5}</style>
